...
Bootstrap and find the values of $\hat{r}^2$ and $log(\hat{\beta_{0}* \beta_{1}})$ for each strap. 
...
The $\hat{r}^2$ falls in the range between `r round(range(pull(parameter_est, r_squared)),3)`. From the plot, we can see that $\hat{r}^2$ is roughly normal distributed with a mean of `r round(mean(pull(parameter_est, r_squared)),3)`, and standard deviation `r round(sd(pull(parameter_est, r_squared)),3)`.


2) $log(\hat{\beta}_0 * \hat{\beta}_1)$ distribution plot
...
The $log(\hat{\beta}_0 * \hat{\beta}_1)$ falls in the range between `r round(range(pull(parameter_est, log_product)),3)`.From the plot, we can see that $log(\hat{\beta}_0 * \hat{\beta}_1)$ appears to be normally distributed with a mean of `r round(mean(pull(parameter_est, log_product)),3)` and standard deviation `r round(sd(pull(parameter_est, log_product)),3)`

**95% confidence interval for the interested quantities**

1) $\hat{r}^2$ 95% Confidence Interval

```{r}
quantile(pull(parameter_est, r_squared), probs = c(0.025,0.975))
```

The 95% confidence of $\hat{r}^2$ is (0.89,0.93)

2) $log(\hat{\beta}_0 * \hat{\beta}_1)$ 95% Confidence Interval

```{r}
quantile(pull(parameter_est, log_product), probs = c(0.025,0.975))
```

The 95% confidence of $$log(\hat{\beta}_0 * \hat{\beta}_1)$$ is (1.96,2.06)